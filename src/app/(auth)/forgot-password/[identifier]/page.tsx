...
function ForgotPassword() {
  ...
  const param = useParams<{ identifier: string }>();
  ...
  const onSubmit = async (data: z.infer<typeof forgotPasswordSchema>) => {
    try {
      ...
      const response = await axios.post("/api/forgot-password", {
        identifier: param.identifier || data.identifier,
      });
      ...
      router.replace(`/verify-forgot-password/${param.identifier}`);
    } catch (error) {
  ...
  useEffect(() => {
    if (param.identifier) {
      form.setValue("identifier", param.identifier);
    }
  }, [form, param.identifier]);
